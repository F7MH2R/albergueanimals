...
import modelos.Usuarios; // Asumiendo que tienes una clase Usuarios que representa al cliente
...
@WebServlet("/RegistrarAdopcionCliServlet")
public class RegistrarAdopcionCliServlet extends HttpServlet {
    @Override
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (cliente == null || !"Cliente".equals(cliente.getRol())) {
            // Redirige a la página de login si no hay un cliente autenticado
        ...
        // Usar los datos del cliente autenticado para el adoptante
        String nombreAdoptante = cliente.getNombreCompleto();
    ...
    private int obtenerOInsertarAdoptante(Connection conn, String nombre, String correo, String telefono, String direccion) throws SQLException {
        String querySelect = "SELECT id_adoptante FROM Adoptantes WHERE nombre = ? AND correo = ?";
        PreparedStatement stmtSelect = conn.prepareStatement(querySelect);
        stmtSelect.setString(1, nombre);
        stmtSelect.setString(2, correo);
